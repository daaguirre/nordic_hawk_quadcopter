...
#include <math.h>
#include "madgwick_sensor_fusion.h"

// System constants
#define deltat 0.1f // sampling period in seconds (shown as 1 ms)
#define gyroMeasError 3.14159265358979 * (40.0f / 180.0f) // gyroscope measurement error in rads/s (start at 40 deg/s)
...
/**
* @brief Madgwick sensor fusion update
* @param[in] w_x, w_y, w_z  gyro x,y and z measurements in
* @param[in] a_x, a_y, a_z  accel x,y and z measurements in G's
* @param[in] m_x, m_y, m_z  magnetometer x,y and z measurements
* @return euler_angles roll, pitch and yaw angles in rad/s
*/
void madgwick_filter_update(float a_x, float a_y, float a_z, float w_x, float w_y, float w_z, float m_x, float m_y, float m_z, float *euler_angles)
{
	...
	float roll_y = 2*(SEq_3SEq_4 - SEq_1SEq_2);
	float roll_x = 2*(SEq_1SEq_1 + SEq_4*SEq_4) - 1;
	euler_angles[0] = atan2f(roll_y, roll_x);

	float pitch_x = 2*(SEq_2SEq_4 + SEq_1SEq_3);
	euler_angles[1] = -asinf(pitch_x);

	float yaw_y = 2*(SEq_2SEq_3-SEq_1SEq_4);
	float yaw_x = 2*(SEq_1SEq_1 + SEq_2*SEq_2) - 1;
	euler_angles[2] = atan2f(yaw_y, yaw_x);
}

////Gets the euler angles in the next format:
//// 0 = roll, 1 = pitch, 2 = yaw
//void get_euler_angles_from_quaternion(float *euler_angles)
//{
//	float roll_y = 2*(SEq_3*SEq_4-SEq_1*SEq_2);
//	float roll_x = 2*(SEq_1*SEq_1 + SEq_4*SEq_4) - 1;
//	euler_angles[0] = atan2f(roll_y/roll_x);
//
//	float pitch_x = 2*(SEq_2*SEq_4 + SEq_1*SEq_3);
//	euler_angles[1] = -asinf(pitch_x);
//
//	float yaw_y = 2*(SEq_2*SEq_3-SEq_1*SEq_4);
//	float yaw_x = 2*(SEq_1*SEq_1 + SEq_2*SEq_2) - 1;
//	euler_angles[1] = atan2f(roll_y/roll_x);
//}
